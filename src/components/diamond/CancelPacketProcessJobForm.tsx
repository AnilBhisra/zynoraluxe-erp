...
import { cancelPacketProcessJobAction } from "@/app/actions/diamond";
import { Button } from "@/components/ui/Button";
import { Alert } from "@/components/ui/Alert";

export function CancelPacketProcessJobForm({ jobId, jobCode }: { jobId: string; jobCode: string }) {
  const [open, setOpen] = useState(false);
  const [state, formAction, pending] = useActionState(cancelPacketProcessJobAction, undefined);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        if (!window.confirm(`Cancel job ${jobCode}? The issued stones go back into their packets and the WIP posting reverses. This cannot be undone.`)) {
          e.preventDefault();
        }
      }}
    ...
    >
      ...
      <input
        ...
        required
        minLength={3}
        placeholder="e.g. Issued to the wrong Manufacturer"
