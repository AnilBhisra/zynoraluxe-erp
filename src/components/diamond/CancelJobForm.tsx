...
import { cancelDiamondJobAction } from "@/app/actions/diamond";
import { Button } from "@/components/ui/Button";
import { Alert } from "@/components/ui/Alert";

export function CancelJobForm({ jobId, jobCode }: { jobId: string; jobCode: string }) {
  const [open, setOpen] = useState(false);
  const [state, formAction, pending] = useActionState(cancelDiamondJobAction, undefined);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        if (!window.confirm(`Cancel job ${jobCode}? The issued rough returns to stock and the WIP posting reverses. This cannot be undone.`)) {
          e.preventDefault();
        }
      }}
    ...
    >
      ...
      <input
        ...
        required
        minLength={3}
        placeholder="e.g. Issued to the wrong Karigar"
